"""Chamadas ao LLM (DeepSeek por padrão, OpenRouter em modo de teste — ambas
APIs OpenAI-compatible, ver app/config.py:LLM_PROVIDER).
...
from app.config import LLM_MODEL, LLM_PROVIDER, llm_client


@dataclass
class UsoNormalizado:
    """Shape que `registrar_uso_cache` espera. Com a DeepSeek: input_tokens ←
    ...
    "cache write" pago à parte, só miss/hit). Outros providers (ex.:
    OpenRouter) não expõem esses campos específicos da DeepSeek — nesse caso
    tudo cai em input_tokens (sem visibilidade de cache) via _normalizar_usage."""
...
def _normalizar_usage(usage) -> UsoNormalizado:
    ...
    extensão específica da DeepSeek — outro provider (ex.: OpenRouter) só
    devolve o usage padrão da OpenAI (prompt_tokens/completion_tokens), sem
    detalhe de cache. getattr com default evita AttributeError nesse caso."""
...
# OpenAI) pra desligar o reasoning mode — não faz sentido mandar pra outro
# provider via OpenRouter (o modelo de teste padrão, aliás, É um modelo de
# reasoning — "nemotron...-reasoning" — então nem se aplicaria da mesma forma).
    ...
    def _gen():
        try:
            stream = llm_client.chat.completions.create(
                ...
                **({"extra_body": _EXTRA_BODY_DEEPSEEK} if LLM_PROVIDER == "deepseek" else {}),
            )
            for chunk in stream:
